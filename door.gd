...
#		get_tree().quit()
	...
	if opened:
		...
		if Main.level == 0:
			...
			if points >= total_count and player_nearby and !opened:
				Main.player.door_opening = true

			#	$Counter.visible = false
				$Counter.text = str(total_count) + "/" + str(total_count)
				if $Counter.modulate.a > 0.0 and opening > -.6:
					...
					$Counter.position.y = 4.28 + sin(Time.get_ticks_msec()/200.0)/6.0
					$Counter.modulate.a += delta*4.0
				...
				$Counter.text = str(Main.points) + "/" + str(total_count)
